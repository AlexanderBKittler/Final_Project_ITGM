package processing.test.final_project;
...
public void mousePressed() { //phone controls...
  if (mouseX < width/2 && mouseY < height*0.75f) {
  ...
  if (mouseY > height*0.75f) { //deletes blocks below hero based on placement
    if (alive == true) {
      if (heros.get(0).x == 50) {
        if (blocks.size() > 0) {
          int i = blocks.size()-1;
...
public void heroDisplay() { //displays hero
  for ( int i=0; i<heros.size(); i++) {
...
public void gridAdd() { //spawns blocks based on numbers generated
  float r = random(10);
  int rx = round(r);
...
public void gridDisplay() { // displays block arrays
...
public void gridUpdate() { //updates the blocks movement
...
class Hero {
  ...
  //**************************************
  public void moveRtY() { //logic behind moving the objects right
    //x=mouseX;
    //y=mouseY;
    //x=x+10;
  ...
  public void moveLfY() { //logic behind moving the objects left
    //x=mouseX;
    //y=mouseY;
    //x=x+10;
    if (x == 50) {
  ...
  public void moveDn() { //logic behind moving objects down
    y=y+100;
  }
}
class Object {
  ...
  //**************************************
  //**************************************
  public void move() { //logic behind moving the objects
    //x=mouseX;
    //y=mouseY;
    //x=x+10;
    if (y < bottom) {
  ...
  //**************************************
  public void BottomDn() { //resize logic for bottom placement
    bottom=bottom+100;
  }

}
  public void settings() {  size ( 1000, 2000 ); }
}
